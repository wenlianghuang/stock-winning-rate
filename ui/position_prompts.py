...
## 四、操作情境推演
**純文字** 須依系統提供的**三種市場情境權重**（加總 100%）撰寫，格式如下：
- 先一句話點出**系統基準判斷**（部位損益分類 + 系統傾向）
- 再列 **3 種市場情境**（延續調節 / 橫盤整理 / 技術反彈），每種須含：
  - **權重百分比**（與系統給定數字一致，勿修改）
  - **主線/次線/尾線**標示（最高權重者為主線）
  - **觸發條件**（若/一旦/條件/則）
  - **建議操作**（與系統給定的操作方向一致；市場情境≠直接加碼/減碼機率）
- **必須對齊系統試算的部位狀態**（見下方部位狀態與情境權重）：
- 大幅獲利 → 須談停利/移動停損/獲利了結或明確論證續抱，並引用系統**停利參考價**（近20日高）
- 小幅獲利 → 須談加碼條件/停利/獲利回吐風險
- 損益兩平 → 須給明確的出場或加碼觸發價
- 虧損 → 須提停損/減碼/出場等防禦手段，並引用系統**停損參考價**（近20日低）與**解套參考價**（均價）
