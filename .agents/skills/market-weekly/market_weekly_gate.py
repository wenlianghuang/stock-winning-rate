#!/usr/bin/env python3
"""Market weekly gate: facts → agy narrative → validate loop."""
...
from market_week_prompts import build_fix_prompt, build_market_week_prompt
from market_week_signals import (
    build_market_week_facts,
    facts_path,
...
from market_week_summary import summary_path, write_summary_json
from validate_market_week import ValidationResult, validate_market_week_report
...
def ensure_paths() -> None:
    root = project_root()
    for sub in ("ui", ".agents/skills/market-weekly", ".agents/skills/tw-stock-report"):
        text = str(root / sub)
...
def ensure_leader_csvs(
    window: WeekWindow,
    *,
    skip_fetch: bool = False,
) -> None:
    """Best-effort: run stock-report for leader universe on week_end."""
    if skip_fetch:
        return
    from market_week_signals import load_leader_universe

    leaders = load_leader_universe()
    ids = [e["id"] for e in leaders]
    if not ids:
        return
    lookback = max(len(window.trading_days), 5)
    script = (
        project_root()
        / ".agents"
        / "skills"
    ...
    print(f"PROGRESS: stock-report leaders ({len(ids)} 檔) @ {window.week_end}", file=sys.stderr)
    subprocess.run(cmd, cwd=project_root(), check=False)


def write_markdown(path: Path, body: str, facts: dict[str, Any]) -> None:
    header = (
        f"# 台股市場週報（{facts.get('week_start')}～{facts.get('week_end')}）\n\n"
        f"> 類股來源：證交所官方類股指數（twse_industry_indices）\n\n"
    )
...
def run_gate(
    *,
    ...
    out_dir = market_output_dir(window.week_end)
    out_dir.mkdir(parents=True, exist_ok=True)
    md_path = out_dir / "tw_market_weekly.md"
    log_path = out_dir / "tw_market_weekly.gate.log"
    rounds_dir = out_dir / "tw_market_weekly.gate.rounds"

    if validate_only:
        if not md_path.exists() or not facts_path(window.week_end).exists():
            print("ERROR: validate-only 需要既有 md 與 facts.json", file=sys.stderr)
            return EXIT_NO_DATA
        facts = json.loads(facts_path(window.week_end).read_text(encoding="utf-8"))
        body = md_path.read_text(encoding="utf-8")
        result = validate_market_week_report(body, facts)
    ...
    ensure_leader_csvs(window, skip_fetch=skip_fetch)
    facts_obj = build_market_week_facts(
        window,
        skip_news=skip_news,
    ...
    base_prompt = build_market_week_prompt(
        facts_summary=facts_summary_for_prompt(facts_obj),
        news_section=facts_obj.news_section,
    )
    ...
    for round_no in range(1, max_rounds + 1):
        ...
        validation = validate_market_week_report(body, facts)
        ...
        if validation.passed:
            ...
            write_summary_json(facts, body, summary_path(window.week_end))
            log_path.write_text(
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="台股市場週報（大盤+權值+類股）")
    parser.add_argument(
        "--as-of",
        default=None,
    ...
    parser.add_argument(
        "--week-end",
        default=None,
        help="強制指定週最後交易日 YYYY-MM-DD（略過 cutover）",
    )
    ...
    parser.add_argument(
        "--resolve-only",
        action="store_true",
        help="只印出 week window JSON",
    )
    args = parser.parse_args(argv)

    try:
        window = resolve_window_or_fail(as_of=args.as_of, week_end=args.week_end)
    except Exception as exc:  # noqa: BLE001
        print(f"ERROR: {exc}", file=sys.stderr)
