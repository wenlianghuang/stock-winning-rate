...
- **區間摘要**：累計法人/主力買賣超、融資券淨變化、成交量/當沖均值、區間漲跌幅、MA5、收盤偏離 MA5、MA20、收盤偏離 MA20
- **大盤脈絡**：加權指數（TAIEX）收盤、當日漲跌幅、MA5/MA20、收盤偏離、區間漲跌幅（全市場共用，供個股相對強弱判定）
...
- 快照 CSV：當日完整籌碼 + 區間趨勢摘要（回看天數、累計買賣超、MA5/MA20 等）
...
| MA20 | 月線另抓最近 20 個交易日收盤價計算（與回看天數無關） |
| 大盤脈絡 | 加權指數 `TAIEX`（FinMind `TaiwanStockPrice`）；當日一次抓取，全部個股共用；失敗僅留空不中斷 |
...
| 非交易日 | 依證交所官方開休市日期表判定；週末或國定假日會自動改用最近一個台股交易日 |
